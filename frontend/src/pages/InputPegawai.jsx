...
const InputPegawai = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/pegawai`, form);
      ...
      <div style={{ marginLeft: 220, padding: 40, width: '100%' }}>
        ...
        <form onSubmit={handleSubmit} style={{ maxWidth: 400 }}>
          <div>
            ...
            <input name="pic" value={form.pic} onChange={handleChange} required />
          </div>
          <div>
            <label>NIP</label><br />
          ...
          <div>
            <label>Tim</label><br />
            <input name="tim" value={form.tim} onChange={handleChange} required />
          </div>
          <div>
            <label>Role di Tim</label><br />
            <select name="role_di_tim" value={form.role_di_tim} onChange={handleChange} required>
          ...
          <div>
            ...
            <select name="seksi" value={form.seksi} onChange={handleChange} required>
              <option value="">-- Pilih --</option>
              <option value="II 1">II 1</option>
              <option value="II 2">II 2</option>
            </select>
          </div>
          <button type="submit" style={{ marginTop: 15 }}>Simpan</button>
        </form>
